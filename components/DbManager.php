...
class DbManager extends BaseManager
{
    ...
    public function checkAccess($userId, $permissionName, $params = [])
    {
        ...
        $assignments = [];
        $strIds = [];
        foreach($structureId as $structure) {
            $strId = @$structure->group_id;
            $strIds[] = @$structure->group_id;
        }
        if (isset($this->checkAccessAssignments[(string) $strId])) {
            $assignments = $this->checkAccessAssignments[(string) $strId];
        } else {
            $assignments = $this->getAssignments($strIds);
            $this->checkAccessAssignments[(string) $strId] = $assignments;
        }       
        ...
        $this->loadFromCache();
        if ($this->items !== null) {
            return @$this->checkAccessFromCache($strId, $permissionName, $params, $assignments);
        }
        return @$this->checkAccessRecursive($strId, $permissionName, $params, $assignments);
    }
    ...
    public function getPermissionsByUser($userId)
    {
        ...
        $userGroups = @$user->userGrup;
        // $strId = [];
        $directPermission = [];
        $inheritedPermission = [];
        foreach($userGroups as $structure) {
            $strId = @$structure->group_id;
            $directPermission[] = $this->getDirectPermissionsByUser($strId);
            $inheritedPermission[] = $this->getInheritedPermissionsByUser($strId);
        }
        // $strId = $structureId->group_id;
    ...
    public function getAssignment($roleName, $userId)
    {
        ...
        $query = (new Query())
            ...
            ->where(['user_id' => (string) $userId]);
        $assignments = [];
        $assignments['alluser'] = new Assignment([
            'userId' => $userId,
            'roleName' => 'alluser',
